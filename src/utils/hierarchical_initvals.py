...
def _get_noncentered_initvals(model_name, n_subjects=None):
    ...
    base_initvals = {
        ...
        'v_Intercept': np.array(-0.02),  # Fixed intercept
        'v_signed_contrast': np.array(0.025),  # Fixed contrast effect
        'v_1|participant_id_sigma': np.array(0.07),  # Random effect variance
        'v_1|participant_id_offset': np.random.normal(0, 0.01, n_subjects),  # Subject deviations (small)
        'v_signed_contrast|participant_id_sigma': np.array(0.002),  # Contrast variance
        'v_signed_contrast|participant_id_offset': np.random.normal(0, 0.001, n_subjects),  # Contrast deviations
        
        # z parameter (starting point) - should be around 0.5
        'z_Intercept': np.array(0.47),  # Fixed starting point
        'z_1|participant_id_sigma': np.array(0.05),  # Random effect variance
        'z_1|participant_id_offset': np.random.normal(0, 0.01, n_subjects),  # Subject deviations
        
        # t parameter (non-decision time) - from fitted: ~0.1
        't_Intercept': np.array(0.1),  # Fixed NDT
        't_1|participant_id_sigma': np.array(0.005),  # Random effect variance
        't_1|participant_id_offset': np.random.normal(0, 0.002, n_subjects),  # Subject deviations
        
        # a parameter (boundary separation) - from fitted: ~0.645
        'a_Intercept': np.array(0.645),  # Fixed boundary
        'a_1|participant_id_sigma': np.array(0.014),  # Random effect variance
        'a_1|participant_id_offset': np.random.normal(0, 0.007, n_subjects),  # Subject deviations
    }
    ...
    if 'prevresp_v' in model_name or 'prevresp_zv' in model_name:
        ...
        base_initvals.update({
            'v_prevresp_cat': np.array([-0.003]),  # Fixed effect (array for categories)
            'v_prevresp_cat|participant_id_sigma': np.array([0.23]),  # Random effect variance
            'v_prevresp_cat|participant_id_offset': np.random.normal(0, 0.01, (n_subjects, 1)),  # Subject deviations
        })
    
    if 'prevresp_z' in model_name or 'prevresp_zv' in model_name:
        base_initvals.update({
            'z_prevresp_cat': np.array([0.021]),  # Fixed effect (array for categories)
            'z_prevresp_cat|participant_id_sigma': np.array([0.024]),  # Random effect variance
            'z_prevresp_cat|participant_id_offset': np.random.normal(0, 0.01, (n_subjects, 1)),  # Subject deviations
        })
    
    # Add theta for angle models
    if model_name.startswith('angle'):
        base_initvals.update({
            'theta_Intercept': np.array(0.6),  # Fixed angle parameter
            'theta_1|participant_id_sigma': np.array(0.4),  # Random effect variance
            'theta_1|participant_id_offset': np.random.normal(0, 0.15, n_subjects),  # Subject deviations
        })
...
def _get_centered_initvals(model_name, n_subjects=None):
    ...
    if model_name.startswith('angle'):
        base_initvals.update({
            'theta_1|participant_id_mu': 0.6,  # Reasonable middle value for angle parameter
            'theta_1|participant_id': np.random.normal(0.6, 0.15, n_subjects),  # Moderate variation
        })
    
    return base_initvals

def get_jittered_initvals(model_name, parameterization='noncentered', n_chains=3, jitter_scale=0.01, n_subjects=None):
    """
    ...
    if n_chains == 1:
        ...
        for param, value in base_initvals.items():
            if isinstance(value, np.ndarray):
                # Jitter array parameters
                chain_initvals[param] = value + np.random.normal(0, jitter_scale, value.shape)
            else:
                # Jitter scalar parameters
                chain_initvals[param] = value + np.random.normal(0, jitter_scale)
...
def get_single_initvals(model_name, parameterization='noncentered', jitter_scale=0.01, n_subjects=None):
    """
